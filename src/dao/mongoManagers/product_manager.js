...
class ProductManager {
  ...
  async getProducts(limit, page, sort, filter) {
    if(!isNaN(limit) && limit >= 0){
      ...
      if(filter){
        return await productModel.paginate({category: filter}, {limit: limit || 5, page: page, sort: {price: sort}, lean: true});
      } else {
        return await productModel.paginate({}, {limit: limit || 5, page: page, sort: {price: sort}, lean: true});
      }
    } else {
